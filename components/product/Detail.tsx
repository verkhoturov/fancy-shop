...
export const Detail = ({ productAcf, reviews } : TDetail) => {
	...
	const [activeTab, setActiveTab] = useState<'desc' | 'rev'>(!!description ? 'desc' : 'rev');
	const [averageRating, setAverageRating] = useState(0)

	useEffect(()=> {
		let count = 0
		const sum = reviews?.map((item: any) => item.rating).reduce((sum: number, item: number, index: number)=> {
			count += item;
					...
					<div className="product-detail__rating rating">
						...
						<div className="rating__stars">
							{[1, 2, 3, 4, 5].map((num: number)=> (
								<svg
									key={num}
									className={`rating__star${ num <= averageRating ? ' rating__star--active' : '' }`}
									width="20"
									height="20"
									viewBox="0 0 20 20"
									fill="none"
