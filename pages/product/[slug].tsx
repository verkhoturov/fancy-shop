...
export const getStaticProps = async ({ params }: GetStaticPropsContext)=> {
	...
	const products = slug ? await fetchWooCommerceProducts('products?per_page=100').catch((error) =>
		console.log(error)
	) : []

	const product = products.data.find((item: any) => item.slug === slug)
	const freshOffers = products?.data.filter((item: any) => item.slug !== slug)
	const reviews = slug ? await fetchWooCommerceProducts(`products/reviews?product=${product.id}`).catch((error) =>
		console.log(error)
	) : []
